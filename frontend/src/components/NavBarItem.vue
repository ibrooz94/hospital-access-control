...
const itemLabel = computed(() =>
  props.item.isCurrentUser ? useAuthStore().user?.email : props.item.label
)
...
<template>
  ...
  <component :is="is" v-else ref="root" class="block lg:flex items-center relative cursor-pointer"
    ...
    <div class="flex items-center" :class="{
      ...
      <UserAvatarCurrentUser v-if="item.isCurrentUser" class="w-6 h-6 inline-flex" />
      <BaseIcon v-if="item.icon" :path="item.icon" class="transition-colors" />
      <span class="transition-colors" :class="{ 'lg:hidden': item.isDesktopNoLabel && item.icon }">{{ itemLabel
        }}</span>
      <BaseIcon v-if="item.menu" :path="isDropdownActive ? mdiChevronUp : mdiChevronDown"
        class="hidden lg:inline-flex transition-colors" />
    </div>
    <div v-if="item.menu"
      class="text-sm border-b border-gray-100 lg:border lg:bg-white lg:absolute lg:top-full lg:left-0 lg:w-48 lg:z-20 lg:rounded-lg lg:shadow-lg lg:dark:bg-slate-800 dark:border-slate-700"
